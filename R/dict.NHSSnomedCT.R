...
build_concept_tables.NHSSnomedCT <- function(dict,replacements) {
  ...
  write_table_from_file(dict$src,"snomed_relationship",find_matching_file(file_path,"_Relationship"),delim="\t",file_col_types="cccccccccc",col_names = c("id", "effectiveTime", "active", "moduleId", "snomed_code",
                                                                                                                                                         "parent_snomed_code", "relationshipGroup", "typeId", "characteristicTypeId",
                                                                                                                                                         "modifierId"),skip_rows=1)
}
...
get_child_codes.NHSSnomedCT<-function(dict,code,immediate_children=F,current_only=F) {
  ...
  if(current_only) {
    code_tbl<-tbl(dict$src,get_ctable_name(dict)) %>% filter(snomed_code %in% codes & active=='1') %>% collect()
