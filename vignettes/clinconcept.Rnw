...
Described here is an extensible \proglang{R} \citep{R} package \pkg{clinconcept} which helps researchers work with the clinical concept dictionaries most commonly used in primary and secondary care in the UK: READ versions 2 and 3 \citep{ONeil:1995}, SNOMED-CT and ICD10. The package provides functionality to i) build a local or remote database containing one or more clinical dictionaries, using UK National Health Service (NHS) provided downloads of these dictionaries; ii) search clinical concepts using a \pkg{dplyr} focused interface to achieve complex searches; and iii) extract child and parent codes/terms with support for hierarchical and DAG-based searches (depending on the dictionary).

\section{Installing the Package} \label{sec:install}

To install the latest version of \pkg{clinconcept} you can install it directly from CRAN.
...
The package uses a number of third-party packages including \pkg{dplyr}, \pkg{RMySQL} and \pkg{RSQLite}. During the install process these dependencies should be installed automatically.
...
Once registered, you will need to request access to, and then download your required code sets (see appendix A for details of the download types currently supported by \pkg{clinconcept}). Once downloaded, you will need to extract the files and make a note of the base location of these files for later use. As of 2020, READ version 2 and 3 downloads are now deprecated and have been withdrawn from the main TRUD site, please see appendix A for more information.
